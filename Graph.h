...
class graph {
  ...
  public:
    ...
    struct vertex_label {
      ...
      int npaths;
        ...
        vertex_label( double _dist=0.0, int _pred=-1, char _state='?', int _npaths=0)
        : dist { _dist }, pred { _pred }, state { _state}, npaths { 0 }, numoutput{0}, numinput{0}
        { }
  ...
  public:
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      // since src is the root of the bfs tree, it has no predecessor.
...
private:
    ...
    void extract_path_HELPER(const vector<vertex_label> &rpt, int dest, vector<int> &path) {
        
        if (rpt[dest].pred == dest) {
            path.push_back(dest);
            return;
        }
        extract_path_HELPER(rpt, rpt[dest].pred, path);
        path.push_back(dest);
    }
...
public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, int dest, vector<int> & path) {
      ...
      // your code here!
        ...
        extract_path_HELPER(rpt, dest, path);
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      if(has_cycle())
        ...
        {
            if (vertices[OP[i]].incoming.size() == 0)
            {
                rpt[OP[i]].pred = OP[i];
                rpt[OP[i]].dist = 0;
            }
            
            for(edge &e : vertices[OP[i]].outgoing)
            {
                int ver = e.vertex_id;
                double weight = e.weight;
                
                if (rpt[ver].dist < rpt[OP[i]].dist + weight )
                {
                    rpt[ver].dist = rpt[OP[i]].dist + weight;
                    rpt[ver].pred = OP[i];
                }
            }
        }
...
private:
    ...
    int NumInput(int source, vector<vertex_label> &rpt)
    {
        if(vertices[source].incoming.size() == 0){
            rpt[source].numinput = 1;
        }
        else
        {
            for (int i = 0; i< vertices[source].incoming.size(); i++)
            {
                int v = vertices[source].incoming[i].vertex_id;
                if(rpt[v].numinput > 0)
                {
                    rpt[source].numinput += rpt[v].numinput;
                    rpt[source].npaths = rpt[source].numinput;
                }
                else
                {
                    rpt[source].numinput += NumInput(v, rpt);
                }
            }
        }
        return rpt[source].numinput;
    }
    
    int NumOutput(int source, vector<vertex_label> &rpt)
    {
        if(vertices[source].outgoing.size() == 0)
        {
            rpt[source].numoutput = 1;
        }
        else
        {
            for (int i = 0; i< vertices[source].outgoing.size(); i++)
            {
                int v = vertices[source].outgoing[i].vertex_id;
                if(rpt[v].numoutput > 0)
                {
                    rpt[source].numoutput += rpt[v].numoutput;
                    rpt[source].npaths = rpt[source].numoutput;
                }
                else
                {
                    rpt[source].numoutput += NumOutput(v, rpt);
                }
            }
        }
        return rpt[source].numoutput;
    }
...
public:
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
        ...
        for (int i=0; i < num_nodes(); i++)
        {
            rpt[i].npaths = rpt[i].numinput * rpt[i].numoutput;
        }
...
public:
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())
      {
        ...
        int check;
        vector<orderedpathstruct> recordkeeping;
        recordkeeping.resize(order.size());
        
        for (int i=0; i < order.size(); i++)
        {
            if (vertices[order[i]].incoming.size() == 0)
            {
                recordkeeping[order[i]].tracked = -999;
                continue;
            }
            ...
            for (int j =0; j < vertices[order[i]].incoming.size(); j++)
            {
                ...
                if (recordkeeping[check].tracked != -999)
                {
                    return false;
                }
            }
...
private:
    ...
    void enum_paths_HELPER(int target, string path, int j, vector<string> & paths) {
        int ver;
        bool fenum = false;
        if (j == target) {
            paths.push_back(path + vertices[j].name);
        }
        else {
            for(edge &e : vertices[j].outgoing) {
                ver = e.vertex_id;
                if (fenum ==false) {
                    path = path + vertices[j].name + " ";
                }
                enum_paths_HELPER(target, path, ver, paths);
                fenum = true;
            }
        }
    }
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      // your code here!
        ...
        for (k = 0; k < vertices.size(); k++) {
            if (vertices[k].incoming.size() == 0) {
                enum_paths_HELPER(target, path, k, paths);
            }
        }
        return true;
    }
